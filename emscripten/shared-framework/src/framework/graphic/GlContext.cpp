...
void GlContext::framebufferTexture2D(uint32_t index, uint32_t textureId) {
  ...
  glCheck(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index,
...
void GlContext::checkFrameBuffer() {
  ...
  if (status == GL_FRAMEBUFFER_UNDEFINED)
    D_THROW(std::runtime_error,
            "incomplete frame buffer GL_FRAMEBUFFER_UNDEFINED");
  ...
  if (status == GL_FRAMEBUFFER_UNSUPPORTED)
    D_THROW(std::runtime_error,
            "incomplete frame buffer GL_FRAMEBUFFER_UNSUPPORTED");
...
void GlContext::vertexAttribPointer(uint32_t attrId, uint32_t rowSize,
...
void GlContext::drawArrays(Primitives primitive, uint32_t primitiveStart,
...
void GlContext::drawArraysInstanced(Primitives primitive,
...
bool GlContext::linkProgram(uint32_t programId, uint32_t vertexShader,
...
void GlContext::uploadPixels(uint32_t width, uint32_t height,
                             const void* pixels) {
  GLint level = 0;
  GLint border = 0;
...
void GlContext::downloadPixels(uint32_t posX, uint32_t posY, uint32_t width,
...
void GlContext::setAsDepthTexture(uint32_t width, uint32_t height) {
  ...
  glCheck(glTexImage2D(GL_TEXTURE_2D, level, internalFormat, GLsizei(width),
                       GLsizei(height), border, format, type, nullptr));
...
GLenum getRawBlendFunc(GlContext::BlendFuncs func) {
  switch (func) {
  case GlContext::BlendFuncs::srcAlpha:
    return GL_SRC_ALPHA;
    break;
  case GlContext::BlendFuncs::oneMinuxSrcAlpha:
...
void GlContext::setBackFaceCullingDirection(
  BackFaceCullingDirection direction) {
  switch (direction) {
...
void GlContext::clearDepth(float value) { glCheck(glClearDepthf(value)); }
