  ...
  for (std::size_t ii = 0; ii < _def.vbos.size(); ++ii)
  {
    ...
    for (const auto& attr : attrs)
    {
      ...
      for (uint32_t kk = 0; kk < totalRows; ++kk)
      {
        ...
        const uint32_t rowIndex = (uint32_t(attr.index) + kk * rowSize) * uint32_t(sizeof(float));
...
void Geometry::render() const
{
  ...
  if (_isInstanced)
    GlContext::drawArraysInstanced(primitive, _primitiveStart, _primitiveCount, _instanceCount);
  else
...
void Geometry::setPrimitiveStart(uint32_t start)
{
  _primitiveStart = start;
}

void Geometry::setPrimitiveCount(uint32_t count)
{
