...
float BSpline::calcAt(float coef, uint32_t currDimension) const
{
  ...
  coef = coef * (margin * 2.0f + float(_def.knotsLength) / float(_def.dimensions));
  ...
  float result = 0.0f;
  for (int32_t ii = coefInterval - _rangeInterval; ii <= coefInterval + _rangeInterval; ++ii)
  {
    float value = 0.0f;
    ...
    if (float(ii) < margin)
      ...
      value = _def.knotsData[int32_t(float(iknotsLength) / float(idimensions) - 1.0f) * idimensions + icurrDimension];
    else
      value = _def.knotsData[int32_t(float(ii) - margin) * idimensions + icurrDimension];
