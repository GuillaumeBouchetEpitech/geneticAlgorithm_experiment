...
{
	return 1.0f + std::sin(1.5707963f * (t - 1.0f));
}
...
{
  return (std::pow(2.0f, 8.0f * t) - 1) / 255.0f;
}
...
{
  return std::pow(2.0f, 6.0f * (t - 1.0f)) * std::abs(std::sin(t * math::pi * 3.5f));
}

float easeOutBounce(float t)
{
  return 1.0f - std::pow(2.0f, -6.0f * t) * std::abs(std::cos(t * math::pi * 3.5f));
}
