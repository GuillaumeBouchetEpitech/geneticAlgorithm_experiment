...
public:
  static_heap_grid_array_iterator(ValueType* ptr)
    : _ptr(ptr)
  {}
...
class static_heap_grid_array
{
public:
  using ValueType = Type;
  using Iterator = static_heap_grid_array_iterator<static_heap_grid_array<Type>>;
  using RowIterator = static_heap_grid_array_row_iterator<static_heap_grid_array<Type>>;
...
public:
  ...
  Type& get(std::size_t row, std::size_t column) { return _data[row * _width + column]; }
  Type& operator()(std::size_t row, std::size_t column) { return get(row, column); }

  const Type& get(std::size_t row, std::size_t column) const { return _data[row * _width + column]; }
  const Type& operator()(std::size_t row, std::size_t column) const { return get(row, column); }
  ...
  // Iterator beginColumns(uint32_t row) { return Iterator(_data + row * _width); }
  // Iterator endColumns(uint32_t row) { return Iterator(_data + (row + 1) * _width); }

  // RowIterator beginRow(uint32_t row) { return RowIterator(_data + row * _width, _width); }
  // RowIterator endRow(uint32_t row) { return RowIterator(_data + (row + 1) * _width, _width); }

  RowIterator beginRow() { return RowIterator(_data.data(), _width); }
  RowIterator endRow() { return RowIterator(_data.data() + _data.size(), _width); }

  Iterator beginColumns(std::size_t row) { return Iterator(_data.data() + row * _width); }
  Iterator endColumns(std::size_t row) { return Iterator(_data.data() + (row + 1) * _width); }
