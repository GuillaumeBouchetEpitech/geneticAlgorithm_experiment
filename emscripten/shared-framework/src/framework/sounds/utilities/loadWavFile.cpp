...
bool load_wav_file_header(std::ifstream& fileStream, std::uint8_t& channels,
  ...
  if (std::strncmp(buffer, "RIFF", 4) != 0)
    D_THROW(
      std::runtime_error,
      "ERROR: file is not a valid WAVE file (header doesn't begin with RIFF)");
...
char* load_wav(const std::string& filename, std::uint8_t& channels,
  ...
  if (!load_wav_file_header(fileStream, channels, sampleRate, bitsPerSample,
                            size))
    D_THROW(std::runtime_error,
