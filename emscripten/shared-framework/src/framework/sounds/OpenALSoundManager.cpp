...
OpenALSoundManager::~OpenALSoundManager() { alcCloseDevice(_device); }
...
void OpenALSoundManager::loadExplosion(const std::string& filename) {
  ...
  ALsizei size;
  char* soundData =
    load_wav(filename, channels, sampleRate, bitsPerSample, size);
...
void OpenALSoundManager::playExplosion(const std::string& filename,
                                       const glm::vec3& pos) {
  if (!_enabled)
    return;
  ...
  _currentSource = (_currentSource + 1) %
                   (_sources.size() - totalCarEngineSources); // <= hacky
...
void OpenALSoundManager::playCarEngine(const glm::vec3& pos, float pitch) {
  ...
  const ALuint currSource =
    _sources[totalSources - totalCarEngineSources + _currentSource_2]; // hacky
