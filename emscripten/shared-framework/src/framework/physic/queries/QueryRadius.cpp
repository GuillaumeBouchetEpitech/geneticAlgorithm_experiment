...
bool QueryRadius::_queryRadius(QueryRadiusParams& params, QueryRadiusParams::ResultRaw& result)
{
  ...
  btTransform tr;
  tr.setIdentity();
  tr.setOrigin(btVector3(params.position.x, params.position.y, params.position.z));
      ...
      int index1)
    {
      ...
      if (colObj0Wrap->m_collisionObject != &_volume)
      {
        if (_result.allBodiesTotal < _result.allBodiesMaxSize)
        {
          ...
          PhysicBody* pBody = static_cast<PhysicBody*>(colObj0Wrap->m_collisionObject->getUserPointer());
      ...
      if (colObj1Wrap->m_collisionObject != &_volume)
      {
        if (_result.allBodiesTotal < _result.allBodiesMaxSize)
        {
          ...
          PhysicBody* pBody = static_cast<PhysicBody*>(colObj1Wrap->m_collisionObject->getUserPointer());
