...
// void PhysicBody::_onContactChange(ContactEvent event, PhysicContactData* contactData, bool isB)
// {
...
//     btRigidBody* pOtherBody = isB ? contactData->pBodyA : contactData->pBodyB;
//     newContact.other = static_cast<PhysicBody*>(pOtherBody->getUserPointer());

//     _contacts.push_back(newContact);
//   }
//   else if (event == ContactEvent::UPDATE)
//   {
//     auto it = std::find_if(_contacts.begin(), _contacts.end(), [contactData](const ContactPoint& cp)
//     {
//       return cp.id == contactData->id;
//     });
//     if (it == _contacts.end())
//     {
//       D_MYLOG("contact was not found during an update, id=" << contactData->id);
//       return;
//     }
...
//     if (it == _contacts.end())
//     {
//       D_MYLOG("contact was not found during a delete, id=" << contactData->id);
//       return;
//     }
//     _contacts.erase(it);
//   }
...
void PhysicBody::clearForces()
{
  _bullet.body->clearForces();
}
...
{
  return _bullet.body->getFriction();
}




void PhysicBody::setUserValue(int userValue)
{
  _userValue = userValue;
}
int PhysicBody::getUserValue() const
{
  return _userValue;
}


void PhysicBody::setUserData(void* userData)
{
  _userData = userData;
}

const void* PhysicBody::getUserData() const
{
  return _userData;
}

void* PhysicBody::getUserData()
{
  return _userData;
}
...
{
  _bullet.body->applyCentralForce(
    btVector3(force.x, force.y, force.z)
  );
}
...
{
  int newFlags = _bullet.body->getCollisionFlags() | btCollisionObject::CF_NO_CONTACT_RESPONSE;
