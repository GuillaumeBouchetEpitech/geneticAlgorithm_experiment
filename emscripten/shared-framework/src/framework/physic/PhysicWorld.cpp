...
PhysicWorld::PhysicWorld()
  : _physicBodyManager(*this)
  , _physicVehicleManager(*this)
  , _raycaster(*this)
  , _queryRadius(*this)
{
  ...
  // PhysicContactEventsHandler::initialise([](ContactEvent event, PhysicContactData* contactData)
  // {
  //   // D_MYLOG("event=" << int(event));

  //   PhysicBody* pPhysicBodyA = static_cast<PhysicBody*>(contactData->pBodyA->getUserPointer());
  //   PhysicBody* pPhysicBodyB = static_cast<PhysicBody*>(contactData->pBodyB->getUserPointer());
...
{
  _bullet.dynamicsWorld->stepSimulation(elapsedTime, int(maxSubSteps), fixedTimeStep);
}
...
Raycaster& PhysicWorld::getRaycaster()
{
  return _raycaster;
}

QueryRadius& PhysicWorld::getQueryRadius()
{
  return _queryRadius;
}
...
// PhysicHingeConstraint* PhysicWorld::addHingeConstraint(const PhysicHingeConstraintDef& def)
// {
...
// {
//   auto it = std::find(_hinge_constraints.begin(), _hinge_constraints.end(), constraint);
//   if (it == _hinge_constraints.end())
//     return;
...
// PhysicFixedConstraint* PhysicWorld::addFixedConstraint(const PhysicFixedConstraintDef& def)
// {
...
// {
//   auto it = std::find(_fixed_constraints.begin(), _fixed_constraints.end(), constraint);
//   if (it == _fixed_constraints.end())
//     return;
