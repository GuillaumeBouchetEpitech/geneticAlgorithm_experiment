...
Scene::initialise() {
  ...
  GlContext::setBlendFunc(
    GlContext::BlendFuncs::srcAlpha, GlContext::BlendFuncs::oneMinuxSrcAlpha);
...
Scene::updateMatrices() {
  ...
  { // scene
    ...
    camera.hud.setOrthographic(
      0.0f, float(camera.viewportSize.x), 0.0f, float(camera.viewportSize.y),
      -10.0f, +10.0f);
