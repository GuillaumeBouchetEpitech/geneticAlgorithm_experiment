...

    {
      ...
      for (unsigned int ii = 0; ii < cylinderQuality; ++ii) {
        ...
        std::array<Vertex, 4> patchVertices{{
          {{+boardHSize.x * 2.0f, radius * currCos, radius * currSin},
           {12.0f, currCoef * 8.0f}},
          {{-boardHSize.x * 2.0f, radius * currCos, radius * currSin},
           {0.0f, currCoef * 8.0f}},
          {{+boardHSize.x * 2.0f, radius * nextCos, radius * nextSin},
           {12.0f, nextCoef * 8.0f}},
          {{-boardHSize.x * 2.0f, radius * nextCos, radius * nextSin},
           {0.0f, nextCoef * 8.0f}},
        }};
...
void BackGroundCylindersRenderer::setMatricesData(
...
void BackGroundCylindersRenderer::render(const glm::vec3& center) {
  ...
  for (std::size_t ii = 0; ii < _textures.size(); ++ii) {
    ...
    model = glm::rotate(model, _cylinderAnimationTime * 0.1f * (ii + 1),
                        glm::vec3(+1, 0, 0));

    const glm::mat4 composed =
      _matricesData.projection * _matricesData.view * model;
