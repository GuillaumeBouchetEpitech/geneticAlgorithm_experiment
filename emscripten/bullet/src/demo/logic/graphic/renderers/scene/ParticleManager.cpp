...
void ParticleManager::render()
{
    ...
    for (auto& particle : _smallExplosionParticles)
    {
        ...
        _smallSphereInstances.emplace_back(particle.position, localScale, particle.color);
    }
    ...
    for (auto& particle : _bigExplosionParticles)
    {
        ...
        _bigSphereInstances.emplace_back(particle.position, localScale, particle.color);
    }
    ...
    if (!_smallSphereInstances.empty() || !_bigSphereInstances.empty())
    {
        ...
        if (!_smallSphereInstances.empty())
        {
            ...
            _geometrySmallSphere.setInstancedCount(uint32_t(_smallSphereInstances.size()));
        ...
        if (!_bigSphereInstances.empty())
        {
            ...
            _geometryBigSphere.setInstancedCount(uint32_t(_bigSphereInstances.size()));
