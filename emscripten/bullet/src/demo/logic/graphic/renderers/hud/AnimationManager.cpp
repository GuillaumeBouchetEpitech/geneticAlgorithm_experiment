...
AnimationManager::Animation& AnimationManager::Animation::operator=(Animation&& other)
{
  if (&other == this)
    return *this;
...
AnimationManager::AnimationRef AnimationManager::push(float delay, float duration, const Callback& callback)
{
...
AnimationManager::AnimationRef AnimationManager::push(AnimationRef animRef, float delay, float duration, const Callback& callback)
{
  remove(animRef);
...
{
  _animationsPool.for_each([elaspedTime](Animation& currAnim) -> void
  {
  ...
  _animationsPool.filter([](Animation& currAnim) -> bool
  {
...
void AnimationManager::render() const
{
  _animationsPool.for_each([](const Animation& currAnim) -> void
  {
    ...
    const float currTime = (currAnim.duration - currAnim.timeLeft) / currAnim.duration;
